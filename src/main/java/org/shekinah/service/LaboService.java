...
public class LaboService extends BaseService<LaboEntity> implements Serializable {
    ...
    protected void handleDependenciesBeforeDelete(LaboEntity labo) {
        ...
        this.cutAllLaboConsultationsAssignments(labo);
        
    }

    // Remove all assignments from all consultation a labo. Called before delete a labo.
    @Transactional
    private void cutAllLaboConsultationsAssignments(LaboEntity labo) {
        entityManager
                .createQuery("UPDATE Consultation c SET c.labo = NULL WHERE c.labo = :p")
                .setParameter("p", labo).executeUpdate();
    }
    ...
    // Find all consultation which are not yet assigned to a labo
    @Transactional
    public List<LaboEntity> findAvailableLabo(ConsultationEntity consultation) {
        Long id = -1L;
        if (consultation != null && consultation.getLabo() != null && consultation.getLabo().getId() != null) {
            id = consultation.getLabo().getId();
        }
        return entityManager.createQuery(
                "SELECT o FROM Labo o where o.id NOT IN (SELECT p.labo.id FROM Consultation p where p.labo.id != :id)", LaboEntity.class)
